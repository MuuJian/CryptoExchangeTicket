...
class BinanceFutures:
    ...
    def get_symbol_price(self, symbol):
        """獲取單個幣種的當前價格"""
        try:
            resp = self.session.get(self.price_url, params={"symbol": symbol}, timeout=5)
            resp.raise_for_status()
    ...
    def get_oi_analysis(self, symbol):
        ...
        try:
            resp = self.session.get(self.data_url, params=params, timeout=5)
            resp.raise_for_status()
            data = resp.json()
            ...
            if oi_1d_amount == 0 or oi_7d_amount == 0:
                return None
    ...
    for config in configs:
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            ...
            for idx, item in enumerate(sorted_data[:50], 1):
                ...
                f.write(
                    f"{idx:<4} {item['symbol']:<15} {oi_str:<22} "
                    f"{item['ch_1d']:>11}% {item['ch_7d']:>11}%\n"
                )
            f.write("=" * 80 + "\n")

        print(f"📊 已導出 {config['name']} 報告: {file_path}")
...
def main():
    ...
    for index, symbol in enumerate(symbols, 1):
        result = api.get_oi_analysis(symbol)
        if result:
            all_results.append(result)
            if abs(result["ch_7d"]) > 50:
                print(
                    f"🔥 異動提醒: {symbol} 7日持倉增長 {result['ch_7d']}% "
                    f"(當前價值: ${result['curr_oi_usdt']:,.0f})"
                )
